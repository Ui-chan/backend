...
class ProcessedUserStatsView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        daily_success_rate_trend_g1 = list(g1_logs.annotate(date=TruncDate('timestamp')).values('date').annotate(success_count=Count(Case(When(is_successful=True, then=1))), total_count=Count('log_id')).annotate(value=F('success_count') * 100.0 / F('total_count')).values('date', 'value').order_by('date'))
        ...
        for log in g1_logs:
            ...
            if level:
                ...
                if log.is_successful:
                    g1_assistance_success[level] += 1
        
        # 오늘 플레이 시간 계산 (게임 1)
        g1_today_sessions = g1_sessions.filter(session_start_time__date=today, session_end_time__isnull=False)
        g1_today_play_duration_agg = g1_today_sessions.aggregate(total=Sum(F('session_end_time') - F('session_start_time')))['total']
        ...
        g1_stats = {
            ...
            'today_play_duration_seconds': g1_today_play_duration_agg.total_seconds() if g1_today_play_duration_agg else 0
        }
        ...
        overall_avg_response_time_g2 = g2_logs.aggregate(avg=Avg('response_time_ms'))['avg'] or 0
        ...
        g2_stats = {
            ...
            'overall_avg_response_time': overall_avg_response_time_g2,
        ...
        daily_g3_success_rate_trend = list(g3_logs.annotate(date=TruncDate('timestamp')).values('date').annotate(success_count=Count(Case(When(is_successful=True, then=1))), total_count=Count('log_id')).annotate(value=F('success_count') * 100.0 / F('total_count')).values('date', 'value').order_by('date'))
        daily_power_data = defaultdict(lambda: {'total': 0, 'count': 0})
        for log in g3_logs:
        ...
        g3_assistance_success = defaultdict(int)
        g3_assistance_total = defaultdict(int)
        g3_assistance_power = defaultdict(lambda: {'total': 0, 'count': 0})
        for log in g3_logs:
            level = session_assistance_map.get(log.session_id)
            power = log.interaction_data.get('throw_power')
            if level:
        ...
        g3_today_play_duration_agg = g3_today_sessions.aggregate(total=Sum(F('session_end_time') - F('session_start_time')))['total']
        ...
        g3_stats = {
            ...
            'today_play_duration_seconds': g3_today_play_duration_agg.total_seconds() if g3_today_play_duration_agg else 0
        }
        
        processed_data = {'game1': g1_stats, 'game2': g2_stats, 'game3': g3_stats }
        
        serializer = ProcessedStatsSerializer(data=processed_data)
